...
def cm_ggnn(batch_size, image_hidden_size, n_steps, learning_rate, G, num_category, opt, i, beta):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(30):
                ...
                if size_ >= batch_size:
                    ...
                    if i % test_interval == 0:
                        ...
                        print("Train Epoch:", '%d' % epoch, "Batch:", '%d' % i,
                              ...
                              "accuracy:", ".{:.9f}".format(accuracy), "auc:", ".{:.9f}".format(auc))
                        print("Epoch:", '%d' % epoch, "finished", "Best accuracy: %f" % best_accuracy,
                              "Best auc: %f" % best_auc,
            ...
            print("Epoch:", '%d' % epoch, "finished", "Best accuracy: %f" % best_accuracy, "Best auc: %f" % best_auc,
                  "Best epoch: %d" % best_epoch)
            print("batch_size: %d, image_hidden_size: %d, n_steps: %d, learning_rate: %f" % (
                batch_size, image_hidden_size, n_steps, learning_rate))
            ...
            print("Test Epoch:", '%d' % epoch, "accuracy:", "{:.9f}".format(accuracy), "auc:", "{:.9f}".format(auc))
...
if __name__ == '__main__':
    ...
    opt = "RMSProp"
    beta = 0.2
    accuracy = cm_ggnn(batch_size, image_hidden_size, n_steps, learning_rate, G, num_category, opt, i, beta)
